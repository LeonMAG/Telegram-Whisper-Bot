...
MAX_FILE_SIZE_MB = 25  # Límite de Whisper
...
# ── Transcripción ─────────────────────────────────────────────
async def transcribe_audio(file_path: str, file_name: str) -> str:
    """Envía el audio a la API de Whisper y devuelve el texto."""
    ...
    if response.status_code != 200:
        ...
        raise Exception(
            f"Error de la API de OpenAI (HTTP {response.status_code}): {error_msg}"
        )
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja notas de voz y archivos de audio."""
    message = update.message

    # Determinar el tipo de audio recibido
    if message.voice:
    ...
    try:
        ...
        # Transcribir
        text = await transcribe_audio(tmp_path, file_name)
...
async def handle_unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Responde a mensajes de texto no reconocidos."""
    await update.message.reply_text(
...
def main():
    ...
    # Audio: notas de voz, archivos de audio, documentos de audio, video notes
    ...
    app.add_handler(MessageHandler(
        ...
        filters.Document.MimeType("video/mp4"),  # video notes enviados como documento
        handle_audio
    ))
